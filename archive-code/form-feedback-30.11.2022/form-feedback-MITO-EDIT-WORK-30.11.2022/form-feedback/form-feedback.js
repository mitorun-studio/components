...
class ItcSubmitForm {
	...
	constructor(target, config = {}) {
		this._attach = {
			index: 0,
			maxItems: config['attachMaxItems'] || 5,// максимум файлов.
	...
	_setStateValidaion(input, state, message) {
		...
		input.closest('.form__item').querySelector('.form__invalid').textContent = '';
		if (state === 'error' || state === 'success') {
			input.classList.add(className);
			input.closest('.form__item').querySelector('.form__invalid').textContent = text;
		}
	}
	...
	_checkValidity() {
		...
		if (elAttach) {
			...
			elAttach.querySelector('.form__invalid').textContent = '';
			...
			if (isRequired && this._attach.items.length === 0) {
				...
				elAttach.querySelector('.form__invalid').textContent = 'Заполните это поле.';
			}
		}
			...
			if (key === 'attach') {
				...
				if (typeof attachs === 'string') {
					if (elAttach.querySelector('[type="file"]').required) {
						elAttach.classList.add('is-invalid');
						elAttach.querySelector('.form__invalid').textContent = attachs;
					}
				} else {
...
// Инициализация формы и настройки:
...
// Появления окна при успешной отправке:
...
// Очистка формы при клике на .form-success__btn :
