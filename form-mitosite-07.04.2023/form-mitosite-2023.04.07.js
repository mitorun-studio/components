...
// Нужно удалить этот код если нет инпута-телефона!
...
// Нужно удалить этот код если нет инпута-загрузки!
...
//------------------------------------------------
// Скрипт для формы связи от К.Болдырев №4 23.12.2022, доработанный в 06.04.2023 Гершевым.
// Этот скрипт отправляет письма с PHP-конфигом PHPMailer от MaxGraph.
// В этой версии скрипта ссылка на пхп-конфиг перенесена из формы в скрипт, и добавлена поддержка инпута-ловушки.

const formSubmit = async (event, url) => {
	...
	// Код для проверки инпута-ловушки. Если нет такого инпута в форме, то надо закомментировать эти две строки:
...
forms.forEach(form => {
	form.addEventListener('submit', function (e) {
		const url = '/fls/phpmailer/' + (form.dataset.php || 'mail.php');
		formSubmit(e, url);
	});
});
...
// Если во второй форме нужно подключить другой PHP-конфиг:
//<form class="form form--2" action="#" method="POST" enctype="multipart/form-data" data-php="mail-2.php">
